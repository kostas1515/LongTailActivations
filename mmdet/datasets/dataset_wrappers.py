...
class ClassBalancedDataset:
    ...
    def __init__(self, dataset, oversample_thr, filter_empty_gt=True,instance=False):
        self.dataset = dataset
        ...
        if instance is False:
            ...
            repeat_factors = self._get_repeat_factors_instance(dataset, oversample_thr)
    ...
    def _get_repeat_factors_instance(self, dataset, repeat_thr):
        """Get repeat factor for each images in the dataset.
        ...
        #   that contain it: f(c)
        category_freq = defaultdict(int)
        instance_freq = pd.read_csv('./lvis_files/idf_1204.csv')['instance_freq'].values.tolist()[1:]
        instance_freq = np.array(instance_freq)
        num_instances = instance_freq.sum()
        num_images = len(dataset)
        
        for k in range(instance_freq.shape[0]):
            category_freq[k+1] = instance_freq[k] / num_instances
