...
class ADARELIIFLoss(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(ADARELIIFLoss, self).__init__()
    ...
    def cross_entropy(self,
        ...
        target = pred.new_zeros(pred.shape[0], pred.shape[1])
        target[torch.arange(pred.shape[0]), label] = 1
        fg_mask = target[...,:-1]
        
        
        q_fg = torch.softmax(pred[...,:-1],dim=-1)
        
        batch_q=[torch.zeros_like(q_fg) for _ in range(dist.get_world_size())]
        dist.all_gather(batch_q,q_fg)
        batch_q=torch.cat(batch_q,axis=0)
        avg_q_fg = batch_q.mean(axis=0).unsqueeze(0)
